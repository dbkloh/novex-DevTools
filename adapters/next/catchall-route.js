// No JSX, ESM-only, safe in node_modules
import { makeGet as ecGet,  makePost as ecPost  } from "../next/export-code.js";
import { makeGet as edGet,  makePost as edPost  } from "../next/export-docs.js";
import { makeGet as docGet, makePost as docPost } from "../next/dev-docs.js";
import { makeGet as tcGet }                        from "../next/technical-changelog.js";
import { makePost as scPost }                      from "../next/summarize-changelog.js";

const getExportCode  = ecGet();
const postExportCode = ecPost();
const getExportDocs  = edGet();
const postExportDocs = edPost();
const getDoc         = docGet();
const postDoc        = docPost();
...
export async function GET(req, ctx) {
  ...
  if (slug.length === 0) {
    return new Response(JSON.stringify({
      ok: true,
      endpoints: ["export-code","export-docs","[filename]","technical-changelog","summarize-changelog"]
